...
function ImgMediaCard(props) {
  ...
  let nivelHabilitado = localStorage.getItem("nivelCheckCuentas");

  return (
    <div>
      <header>
        <HeaderBar Titulo={props.Titulo} BackTo="/listado" />
      </header>
      <div className="row">
        <Card className={classes.root}>
          <CardActionArea
            onClick={() => history.push("/listado/checkCuentas/lvl1")}
          >
            <CardMedia
              component="img"
              height="max"
      ...
      <div className={`row${nivelHabilitado >= 2 ? "" : "bloqueado"}`}>
        <Card className={classes.root}>
          ...
          >
            <CardMedia component="img" height="max" image={sumayresta} />
            <CardContent>
              <Typography gutterBottom variant="h5" component="h2">
